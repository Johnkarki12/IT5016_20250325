class club_membership:
    ...
    def request_add_on(self):
        num = int(input("How many add-ons do you want: "))
        for i in range(num):
    ...
    def display(self):
        ...
        for add_on, price in self.add_on.items():
            print(f"{add_on}: {price}")


# Example of using the class
name = input("Enter your name: ")
membership = club_membership(name)
membership.request_add_on()  # Request add-on items once
membership.approve()  # Approve the membership based on the total amount
membership.display()  # Display membership details
